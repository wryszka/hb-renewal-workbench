...
Self-contained description of the app + demo for an external reviewer who cannot run the app or read the code, written to be inspected through three personas (H&B actuary/underwriter, data-platform lead, exec sponsor). Facts only; verified against a fresh Reset of DEV (`lr_dev_aws_us_catalog.hb_renewal`, warehouse `a3b61648ea4809e3`) on 2026-08-24, and **re-verified endpoint-by-endpoint after the WP1/WP2/WP3 additions** (self-funded path · quarantine resolution · version diff) the same day — every surface, count, and headline number below was read back from the live endpoints/tables in clean-reset state (54 docs / 49 scenarios / 5 findings / 15 audit events). Where something is stubbed / precomputed / seeded / synthetic / flag-gated, it is flagged in place. All data synthetic (carrier "Meridian Assurance", employer "Harborview Logistics", etc.). Live app: `https://hb-renewal-workbench-7474656169654171.aws.databricksapps.com`.
...
- **KPI tiles [VIEW/STORED]:** In the book **54** · Active **50** · Needs attention **2** (quarantined 1 + differs 1) · Avg carrier action **25.84%** (`avg(5_scenario.baseline_action)` = 0.2584) · Value negotiated **$12,975,304** (sum of `5_scenario.value_at_stake_annual`, status saved/approved).
...
### Quarantine resolution view (WP2) (`/api/quarantine/{id}` + accept/reject/history)
- **Purpose:** turn a held file into an active one — or reject it — with an audit trail.
- **Reached from:** Book (a `quarantined` row) or the Daily-view attention list. Seeded target `DOC-Q900` (Cascade Care / Fernbrook Retail, 15/17).
- **Content [STORED]:** an **expected-vs-found** table per field (template anchor / deterministic value / AI value + the AI-read label / status) and a **proposed re-map** — verified for `DOC-Q900`: `member_months` ← "Covered Life-Months", `annual_trend` ← "Trend Rate (annual)".
- **Actions:** **Accept mapping & re-process** → `POST /api/quarantine/accept` → `accept_remap` (new append-only carrier-template version, re-processes the archived original to active, audits `template_updated → ingested/archived → reprocessed → signed_off`); **Reject with reason** → `POST /api/quarantine/reject` (prompt; `status='rejected'` + audit); **Template history** → `GET /api/template/history?carrier=` renders versions inline.
- **Clean-state note:** before any Accept there is **no Cascade template** yet, so `quarantine.template_version` is `null` and `GET /api/template/history?carrier=Cascade Care` returns `[]` (verified) — the Cascade **v2** row appears only after an Accept. The Meridian template (2 rows) is the pre-seeded one.
...
**1 Daily view** — Overview→Daily view. Screen: hero card "Harborview Logistics — Meridian Assurance · 2026H2", tiles 54/50/2/25.84%/$12,975,304, attention list (Cascade Fernbrook quarantined, Summit Oakmont differs), recent activity.
...
- **Scenario lifecycle:** a saved scenario **cannot be edited, deleted, or approved** in the app (insert-only); seeded history is `approved`, live saves are `saved`. Verdict: holds-with-caveat → *"Decisions are append-only and audited; approval workflow is roadmap."*
- **Version diff (WP3):** opens the hero → the **Version history** panel shows v0 (superseded, 0.1385) → active (0.122); the diff isolates `annual_trend` and the impact banner reads **28.22% → 26.95%, −$58,685/yr** — both actions recomputed by `fn_renewal_buildup` on each version's own inputs (verified `/api/versions`: `old_action` 0.28222, `new_action` 0.26949, `premium_delta_annual` −58,684.58). Opening the **superseded** v0 shows any decision on it with a **"data moved"** chip + one-click recompute onto the active version (append-only). Verdict: holds. **Misread risk:** the panel action is `quoted_change`, not `blended_rate_action` (equal here as adjustment=0).
- **Self-funded (WP1):** built and **governed** (`fn_selffunded_projection`), but **flag-gated** — with `ENABLE_SELFFUNDED` off (the demo default) the actuary finds **no control and `/api/selffunded` returns 404** (verified). With the flag on: a payoff card (FI $5.85M vs SF $5.37M → save $482k/8.2%, max liability $6.51M) and an FI/SF selector on save. The stop-loss/admin PMPMs are **labelled assumptions**. Verdict: holds-with-caveat → *"The self-funded payoff is built and governed by a UC function; it's off until we set the stop-loss numbers with you."*
...
- **App SP grants:** `b417c702-…` has `USE`/`SELECT`/`EXECUTE`/`MODIFY` on the schema, `CAN_USE` warehouse, `CAN_RUN` Genie, `CAN_READ` dashboard, `CAN_MANAGE_RUN` on the reseed Job. **The app can write:** `5_scenario`, `5_gov_audit_event`, `0_carrier_template` (WP2 Accept writes a new version), the source tables (via upload/scan/reset/reprocess), and the Volume archive — all under `MODIFY`. No delete path exposed; **all writes are append/insert or status-flip, never destructive** — WP2 Accept keeps the old template version and quarantined doc (→ `superseded`), WP3 recompute appends a new `5_scenario` (original retained).
- **New endpoints (WP1/2/3) inspected:** `GET /api/versions/{id}`, `GET /api/quarantine/{id}`, `POST /api/quarantine/accept`, `POST /api/quarantine/reject`, `GET /api/template/history`, `POST /api/scenario/recompute-latest`, `POST /api/selffunded` (404 unless `ENABLE_SELFFUNDED`). Each read is the governed warehouse; `fn_selffunded_projection` is a governed UC function (no self-funded math in the container). Guards verified: recompute-latest rejects a scenario already on the latest version (409); accept re-processes under the file's **original** name (the `{doc_id}__` archive prefix would otherwise corrupt `identify()`).
...
**(c) Cosmetic**
11. **Lineage chain nodes look clickable but aren't** (only Download/Open act). *"The chain's the map; the file's the click."*
12. **Lineage shows current table version, not compute-time.** *"That's the current governed version; Delta history is the full audit."*
13. **Inputs escaped, not parameterised** (`sq()`). *"Inputs are escaped; parameter binding is a hardening item."*
14. **`1_detailed_rates` / benefits are illustrative** (per-contract, not reconciled to the PMPM build-up). *"Rate detail is illustrative; the build-up is the governed number."*
